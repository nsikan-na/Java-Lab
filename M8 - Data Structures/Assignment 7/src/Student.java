...
public class Student {
    ...
    public void calcQuizAverage() {
        ...
        quizAverage = total / newQuizArr.length;
    }
    ...
    public void calcHwAverage() {
        ...
        for (int i = 0; i < homeworkArr.length; i++) {
            if (homeworkArr[i] != lowest) {
                newHomeworkArr[newIndex] = homeworkArr[i];
                newIndex++;
            }
        }

        double total = 0;
        for (int homework : newHomeworkArr) {
            total += homework;
        }

        homeworkAverage = total / newHomeworkArr.length;
    }


    public void calcOverallAverage() {
        overallAverage = (quizAverage * .4) + (homeworkAverage * .1) + (midtermGrade * .2) + (finalExamGrade * .3);
    }
